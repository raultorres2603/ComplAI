...
## Instructions
1. **Delegate to planner-agent**: Send a clear prompt to the planner-agent to create or update a `task.md` plan. Wait for the plan before proceeding.
2. **Delegate to builder-agent**: Once the plan is received, send a clear prompt to the builder-agent to implement the plan. Wait for the build results.
3. **Verify results**: Evaluate the builder-agent's report against the `task.md` plan. Check if all steps were completed and if tests passed.
4. **Handle outcomes**:
...
5. **Iterate**: Repeat the delegate-verify-escalate loop until SUCCESS or until the planner-agent indicates that the request cannot be fulfilled.
