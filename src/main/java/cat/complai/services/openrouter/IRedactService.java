...
/**
 * Role interface for complaint redaction operations.
 *
 * <p>Part of the Interface Segregation refactoring of {@link IOpenRouterService}.
 * Clients that only need redaction (e.g. {@code OpenRouterController})
 * should depend on this interface rather than the composite
 * {@link IOpenRouterService}.
 */
public interface IRedactService {
     ...
     * municipality the request is for.
     */
     ...
     *                       the missing fields before drafting. When the user
     *                       explicitly requests anonymity the service rejects with
     *                       VALIDATION — the Ajuntament does not accept anonymous
     *                       complaints.
     * @param cityId         city identifier from the caller's JWT, used to load the
     *                       correct procedures context for RAG
     */
