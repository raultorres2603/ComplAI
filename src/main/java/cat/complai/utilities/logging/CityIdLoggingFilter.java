 ...
 *
 * <p>This filter runs after JwtSessionAuthFilter which sets the "city" attribute.
 * It reads that attribute and makes it available in all logs via SLF4J MDC.
 *
 * <p>MDC is cleared on each new request.
 * Only active when api.key.enabled=true (matching JwtSessionAuthFilter behavior).
 */
...
public class CityIdLoggingFilter {
    ...
    public MutableHttpResponse<?> filter(MutableHttpRequest<?> request) {
        ...
        // Extract cityId from request attributes (set by JwtSessionAuthFilter)
        request.getAttribute(AuthConstants.CITY_ATTRIBUTE, String.class)
